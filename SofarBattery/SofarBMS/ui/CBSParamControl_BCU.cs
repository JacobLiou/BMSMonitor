...
{
    public partial class CBSParamControl_BCU : UserControl
    {
                    ...
                    if (((CheckBox)c).Checked)
                    {
                        switch (c.Name)
                        {
                            ...
                            case "ckb_6":
                                ...
                                text.Text = "0";
                                bytes = Uint16ToBytes(text, text, txt_21, txt_22, 1, 1, 1, 1);
                                break;
                            case "ckb_7":
                                canid[2] = 0xF1;
                                bytes = MixDataToBytes(txt_23, txt_24, txt_25, txt_26, 0.1, 0.1, 1, 1);
                                break;
        ...
        private byte[] Uint16ToBytes(TextBox t1, TextBox t2, TextBox t3, TextBox t4, double scaling1, double scaling2, double scaling3, double scaling4)
        {
            byte[] b1 = MyCustomConverter.Uint16ToByteArray(Convert.ToUInt32(float.Parse(t1.Text) / scaling1));
            byte[] b2 = MyCustomConverter.Uint16ToByteArray(Convert.ToUInt32(float.Parse(t2.Text) / scaling2));
            byte[] b3 = MyCustomConverter.Uint16ToByteArray(Convert.ToUInt32(float.Parse(t3.Text) / scaling3));
            byte[] b4 = MyCustomConverter.Uint16ToByteArray(Convert.ToUInt32(float.Parse(t4.Text) / scaling4));

            return new byte[] { b1[0], b1[1], b2[0], b2[1], b3[0], b3[1], b4[0], b4[1] };
        }
        private byte[] MixDataToBytes(TextBox t1, TextBox t2, TextBox t3, TextBox t4, double scaling1, double scaling2, double scaling3, double scaling4)
        {
            byte[] b1 = MyCustomConverter.Int16ToByteArray(Convert.ToInt16(float.Parse(t1.Text) / scaling1));
            byte[] b2 = MyCustomConverter.Int16ToByteArray(Convert.ToInt16(float.Parse(t2.Text) / scaling2));
            byte[] b3 = MyCustomConverter.Uint16ToByteArray(Convert.ToUInt32(float.Parse(t3.Text) / scaling3));
            byte[] b4 = MyCustomConverter.Uint16ToByteArray(Convert.ToUInt32(float.Parse(t4.Text) / scaling4));
