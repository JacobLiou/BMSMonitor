...
namespace SofarBMS.UI
{
    public partial class CBSControl_BCU : UserControl
    {
        ...
        public List<FaultInfo> FaultNotify_DCDC = new List<FaultInfo>() {
                    new FaultInfo("电感电流瞬时过流保护(PCU),BYTE1>>bit0",1,0,0,0,1),
                    new FaultInfo("电感电流CBC过流保护(PCU),BYTE1>>bit1",1,1,0,0,1),
                    new FaultInfo("放电电流瞬时过流保护(PCU),BYTE1>>bit2",1,2,0,0,1),
                    new FaultInfo("放电电流CBC过流保护(PCU),BYTE1>>bit3",1,3,0,0,1),
                    new FaultInfo("充电电流瞬时过流保护(PCU),BYTE1>>bit4",1,4,0,0,1),
                    new FaultInfo("充电电流CBC过流保护(PCU),BYTE1>>bit5",1,5,0,0,1),
                    new FaultInfo("电池电压瞬时过压保护(PCU),BYTE1>>bit6",1,6,0,0,1),
                    new FaultInfo("电池电压瞬时欠压保护(PCU),BYTE1>>bit7",1,7,0,0,1),
                    new FaultInfo("电池电压硬件过压保护(PCU),BYTE2>>bit0",2,0,0,0,1),
                    new FaultInfo("母线电压瞬时过压保护(PCU),BYTE2>>bit1",2,1,0,0,1),
                    new FaultInfo("母线电压瞬时欠压保护(PCU),BYTE2>>bit2",2,2,0,0,1),
                    new FaultInfo("母线短路保护(PCU),BYTE2>>bit3",2,3,0,0,1),
                    new FaultInfo("BCU-COM过压过流故障(PCU),BYTE2>>bit4",2,4,0,0,1),
                    ...
                    new FaultInfo("接触器短路故障(PCU),BYTE3>>bit0",3,0,0,0,1),
                    new FaultInfo("接触器断路故障(PCU),BYTE3>>bit1",3,1,0,0,1),
                    new FaultInfo("电流采样错误(PCU),BYTE3>>bit2",3,2,0,0,1),
                    new FaultInfo("电感电流平均值过流保护(PCU),BYTE3>>bit3",3,3,0,0,1),
                    new FaultInfo("放电电流平均值过流保护(PCU),BYTE3>>bit4",3,4,0,0,1),
                    new FaultInfo("充电电流平均值过流保护(PCU),BYTE3>>bit5",3,5,0,0,1),
                    new FaultInfo("电池电压平均值过压保护(PCU),BYTE3>>bit6",3,6,0,0,1),
                    new FaultInfo("电池电压平均值欠压保护(PCU),BYTE3>>bit7",3,7,0,0,1),
                    new FaultInfo("母线电压平均值过压保护(PCU),BYTE4>>bit0",4,0,0,0,1),
                    new FaultInfo("母线电压平均值欠压保护(PCU),BYTE4>>bit1",4,1,0,0,1),
                    new FaultInfo("Can通信故障(PCU),BYTE4>>bit2",4,2,0,0,1),
                    new FaultInfo("SCI通信故障(PCU),BYTE4>>bit3",4,3,0,0,1),
                    new FaultInfo("环境温度过温保护(PCU),BYTE4>>bit4",4,4,0,0,1),
                    new FaultInfo("散热器温度过温保护(PCU),BYTE4>>bit5",4,5,0,0,1),
                    new FaultInfo("硬件版本错误保护(PCU),BYTE4>>bit6",4,6,0,0,1),
                    new FaultInfo("充电电流监测保护(PCU),BYTE4>>bit7",4,7,0,0,1),
                    new FaultInfo("电感电流平均值过载保护(PCU),BYTE5>>bit0",5,0,0,0,1),
                    new FaultInfo("放电电流平均值过载保护(PCU),BYTE5>>bit1",5,1,0,0,1),
                    new FaultInfo("母线功率端子P+过温保护(PCU),BYTE5>>bit2",5,2,0,0,1),
                    new FaultInfo("母线功率端子P-过温保护(PCU),BYTE5>>bit3",5,3,0,0,1),
                    new FaultInfo("堆叠连接器端子过温保护(PCU),BYTE5>>bit4",5,4,0,0,1),
                    ...
                    new FaultInfo("母线短路永久故障(PCU),BYTE7>>bit0",7,0,0,0,1),
                    new FaultInfo("母线反接永久故障(PCU),BYTE7>>bit1",7,1,0,0,1),
                    new FaultInfo("预留(PCU),BYTE6>>bit2",6,2,0,0,1),
                    new FaultInfo("预留(PCU),BYTE6>>bit3",6,3,0,0,1),
                    new FaultInfo("预留(PCU),BYTE6>>bit4",6,4,0,0,1),
                    new FaultInfo("预留(PCU),BYTE6>>bit5",6,5,0,0,1),
