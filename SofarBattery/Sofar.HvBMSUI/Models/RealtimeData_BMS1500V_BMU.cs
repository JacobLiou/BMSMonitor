...
namespace PowerKit.UI.Models
{
    public class RealtimeData_BMS1500V_BMU
    {
        ...
        {
            return @"记录时间,电池ID,故障信息,告警信息,保护信息,提示信息,电池状态,BMS状态,允许充电,允许放电,切断继电器,BMU关机,请求强充,充满,放空,编址输入电平,补电输入电平,加热请求,电池电压(V),负载电压(V)," +
                   @"电池电流(A),电池剩余容量(SOC),电池健康程度(SOH),剩余容量(Ah),满充容量(Ah),充电电流上限(A),放电电流上限(A),累计放电量(Wh),累计充电量(Wh),累计放电容量(Ah),累计充电容量(Ah),最高单体电压编号,最高单体电压(mV),最低单体电压编号," +
                   @"最低单体电压(mV),单体电压差(mV),环境温度(℃),Mos温度(℃),功率正端子温度(℃),功率负端子温度(℃),主动均衡温度1(℃),主动均衡温度2(℃),最高单体温度编号,最高单体温度(℃),最低单体温度编号,最低单体温度(℃),均衡母线电压(V)," +
                   @"均衡电流(A),主动均衡最大单体电压(mA),电芯平均温度(℃),主动均衡查表SOC(%),主动均衡累计容量(Ah),主动均衡剩余容量(Ah),辅源电压(V),充电电流偏压(V),放电电流偏压(V),复位方式,放空同步下降SOC,主动均衡状态,标定容量(Ah),供应商名称,锂电池类型";
        }
